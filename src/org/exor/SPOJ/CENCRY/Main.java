...
public class Main {
	public void solve() throws Exception {
		...
		for (int testCase = 0, T = nextInt(); testCase < T; testCase++) {
			Arrays.fill(freq, 0);
			sb.setLength(0);
			char[] text = nextLine().toCharArray();

			for (int i = 0, l = text.length; i < l; i++) {
				char c = text[i];
				int k = freq[c]++;
				char[] containing = consonants, notcontaining = vowels;
				if (c == 'a' || c == 'e' || c == 'i' || c == 'o' || c == 'u') {
					containing = vowels;
					notcontaining = consonants;
				}
				int pos = Arrays.binarySearch(containing, c);
				char e = notcontaining[(pos + (containing.length * k))
						% notcontaining.length];
				sb.append(e);
			}
