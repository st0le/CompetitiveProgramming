package org.exor.SPOJ.ENCONDIN;
    ...
    public void solve() throws Exception {
        ...
        try {
            while((line = nextLine(false)) != null) {
                ...
                int i, j, l;
                int non_repeating_start = -1, non_repeating_end = -1;
                for(i = 0, l = text.length; i < l;) {
                    for(j = 0; j < 9 && i + j < l && text[i] == text[i + j]; j++)
                        ;
                    if(j == 1) {
                        if(non_repeating_start == -1) {
                            non_repeating_start = i;
                            non_repeating_end = i + 1;
                        }
                        else {
                            non_repeating_end++;
                        }
                        ...
                        // fill non repeating first
                        if(non_repeating_start > -1) {
                            sb.append('1');
                            for(int k = non_repeating_start; k < non_repeating_end; k++) {
                                if(text[k] == '1')
                                    sb.append(text[k]);
                                sb.append(text[k]);
                            }
                        ...
                        non_repeating_start = non_repeating_end = -1;
                        sb.append(j).append(text[i]);
                    }
                    i += j;
                }
                // fill non repeating first
