...
public class Main {
    ...
    public void solve() throws Exception {
        ...
        int[] phi = sievePhi(1000000 + 1);
        for(int testCase = 0, T = nextInt(); testCase < T; testCase++) {
            out.println(phi[nextInt()]);
        }
        // ----- solution ends here -----
        // printRuntimeInfo();
        out.flush();
    ...
    private int[] sievePhi(int N) {
        int[] phi = new int[N];
        int[] prime = new int[N / 10];
        int sz = 0;
        BitSet mark = new BitSet(N);
        for(int i = 2; i < N; i++) {
            if(!mark.get(i)) {
                phi[i] = i - 1;
                prime[sz++] = i;
            }
            for(int j = 0; j < sz && prime[j] * i < N; j++) {
                mark.set(prime[j] * i);
                if(i % prime[j] == 0) {
